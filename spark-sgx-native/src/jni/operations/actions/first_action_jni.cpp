...
// FirstAction JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteFirstAction(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("FirstAction", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI FirstAction failed: " + std::string(e.what()));
        return nullptr;
    }
}
