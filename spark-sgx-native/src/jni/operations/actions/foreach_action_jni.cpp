...
// ForeachAction JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteForeachAction(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("ForeachAction", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI ForeachAction failed: " + std::string(e.what()));
        return nullptr;
    }
}
