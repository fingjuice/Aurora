...
// CountAction JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteCountAction(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("CountAction", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI CountAction failed: " + std::string(e.what()));
        return nullptr;
    }
}
