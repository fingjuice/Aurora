...
// CollectAction JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteCollectAction(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("CollectAction", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI CollectAction failed: " + std::string(e.what()));
        return nullptr;
    }
}
