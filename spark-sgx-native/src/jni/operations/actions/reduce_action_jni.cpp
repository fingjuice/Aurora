...
// ReduceAction JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteReduceAction(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("ReduceAction", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI ReduceAction failed: " + std::string(e.what()));
        return nullptr;
    }
}
