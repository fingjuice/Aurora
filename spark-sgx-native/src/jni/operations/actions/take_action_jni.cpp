...
// TakeAction JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteTakeAction(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("TakeAction", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI TakeAction failed: " + std::string(e.what()));
        return nullptr;
    }
}
