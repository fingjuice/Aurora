...
// JoinedRDD JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteJoinedRDD(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("JoinedRDD", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI JoinedRDD failed: " + std::string(e.what()));
        return nullptr;
    }
}
