...
// GroupedRDD JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteGroupedRDD(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("GroupedRDD", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI GroupedRDD failed: " + std::string(e.what()));
        return nullptr;
    }
}
