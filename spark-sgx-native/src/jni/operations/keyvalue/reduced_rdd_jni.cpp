...
// ReducedRDD JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteReducedRDD(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("ReducedRDD", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI ReducedRDD failed: " + std::string(e.what()));
        return nullptr;
    }
}
