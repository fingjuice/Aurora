...
// DistinctRDD JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteDistinctRDD(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("DistinctRDD", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI DistinctRDD failed: " + std::string(e.what()));
        return nullptr;
    }
}
