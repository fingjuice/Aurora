...
// IntersectionRDD JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteIntersectionRDD(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("IntersectionRDD", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI IntersectionRDD failed: " + std::string(e.what()));
        return nullptr;
    }
}
