...
// UnionRDD JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteUnionRDD(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("UnionRDD", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI UnionRDD failed: " + std::string(e.what()));
        return nullptr;
    }
}
