...
// MapPartitionsRDD JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteMapPartitionsRDD(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("MapPartitionsRDD", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI MapPartitionsRDD failed: " + std::string(e.what()));
        return nullptr;
    }
}
