...
// FilteredRDD JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteFilteredRDD(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("FilteredRDD", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI FilteredRDD failed: " + std::string(e.what()));
        return nullptr;
    }
}
