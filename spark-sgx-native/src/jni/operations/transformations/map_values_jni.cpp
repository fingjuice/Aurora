...
// MapValues JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteMapValues(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("MapValues", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI MapValues failed: " + std::string(e.what()));
        return nullptr;
    }
}
