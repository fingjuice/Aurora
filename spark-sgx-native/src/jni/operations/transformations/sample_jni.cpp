...
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteSample(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("Sample", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI Sample failed: " + std::string(e.what()));
        return nullptr;
    }
}
