...
// Coalesce JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteCoalesce(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("Coalesce", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI Coalesce failed: " + std::string(e.what()));
        return nullptr;
    }
}
