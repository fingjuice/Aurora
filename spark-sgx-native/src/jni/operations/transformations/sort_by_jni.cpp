...
// SortBy JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteSortBy(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("SortBy", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI SortBy failed: " + std::string(e.what()));
        return nullptr;
    }
}
