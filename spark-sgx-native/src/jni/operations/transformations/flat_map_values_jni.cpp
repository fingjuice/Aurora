...
// FlatMapValues JNI implementation
JNIEXPORT jobject JNICALL Java_org_apache_spark_sgx_SGXJNIWrapper_nativeExecuteFlatMapValues(
    ...
    try {
        ...
        std::vector<std::string> result = g_enclave->executeCompute("FlatMapValues", input, operation);
        
        return vectorToJavaList(env, result);
        
    } catch (const std::exception& e) {
        SGXUtils::logError("JNI FlatMapValues failed: " + std::string(e.what()));
        return nullptr;
    }
}
